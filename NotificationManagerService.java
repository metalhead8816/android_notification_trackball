...
class NotificationManagerService extends INotificationManager.Stub
{
    ...
    private void updateLightsLocked()
    {
        ...
        if (mLedNotification == null) {
            Log.i(TAG, "Trackball notification seen");

            // get next notification, if any
            int n = mLights.size();
            if (n > 0) {
                if (mCurrentLightIndex >= n)
                    mCurrentLightIndex = 0;
                mLedNotification = mLights.get(mCurrentLightIndex);
                mCurrentLightIndex++;

                int ledARGB = mLedNotification.notification.ledARGB;
                Log.i(TAG, "Trackball color is: " + (ledARGB >> 16) + " " + (ledARGB >> 8) + " " + (ledARGB & 0xFF));
            }
        }
        if (mLedNotification == null) {
